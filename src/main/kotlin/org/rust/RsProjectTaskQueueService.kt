 ...
 * tasks in the queue (See [RsTaskExt.eatStrategy]).
 */
@Service
class RsProjectTaskQueueService(project: Project) {
    private val queue: RsBackgroundTaskQueue = RsBackgroundTaskQueue(project)

    /** Submits a task. A task can implement [RsTaskExt] */
    fun run(task: Task.Backgroundable) = queue.run(task)

    /** Equivalent to running an empty task with [RsTaskExt.eatStrategy] = [eatStrategy] */
    fun eatTasks(eatStrategy: RsTaskExt.EatStrategy) = queue.eatTasks(eatStrategy)
...
interface RsTaskExt {
    @JvmDefault
    val eatStrategy: EatStrategy
        get() = EatStrategy.INDEPENDENT
     ...
     * priority eats (cancels) other tasks with lower or equal priority if [canBeEaten] == true.
     * E.g. [CARGO_SYNC] eats [MACROS_UNPROCESSED] but not [MACROS_CLEAR] or itself.
     * [MACROS_UNPROCESSED] eats itself, [MACROS_FULL] and subsequent.
     */
    enum class EatStrategy(val canBeEaten: Boolean = true) {
        CARGO_SYNC(canBeEaten = false),
        MACROS_CLEAR(canBeEaten = false),
        MACROS_UNPROCESSED,
        MACROS_FULL,
        MACROS_WORKSPACE,

        /** Can't be eaten, eats nothing. Should be the last variant of the enum. */
        INDEPENDENT(canBeEaten = false);

        fun canEat(other: EatStrategy): Boolean =
            other.canBeEaten && this.ordinal <= other.ordinal
    }
}

/** Inspired by [BackgroundTaskQueue] */
private class RsBackgroundTaskQueue(project: Project) {
    ...
    @Synchronized
    fun eatTasks(eatStrategy: RsTaskExt.EatStrategy) {
        cancelableTasks.removeIf { data ->
            if (data.task is RsTaskExt && eatStrategy.canEat(data.task.eatStrategy)) {
                data.cancel()
                true
            } else {
                false
